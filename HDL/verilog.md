...
- 组合逻辑  
	...
	- 连续赋值语句  
	- 电平敏感的周期性行为  
> 无反馈的基本门网表或连续赋值语句可以综合成无锁存功能的组合逻辑  

> 带反馈的条件操作符的连续赋值语句可以综合成一个锁存器  

- 举例  
~~~
assign data_out = (CS_b == 0) ? (WE_b == 0)?data_in：data_out:1'bz;
//写不使能，data_out=data_out,锁存功能；否则data_out=data_in  
//片选不使能，输出三态高阻状态，相当于断开状态，用于总线的连接   
~~~

- 锁存器的无意识综合  
> 组合逻辑的verilog描述必须针对所有可能的输入值给出输出赋值  

- 隐含的锁存器  
- 锁存器的有意识综合  
- 三态器件的综合和总线接口  
- 允许多个器件之间共享总线，三态逻辑值z的分支  
- 

- 6.5 带有触发器的时序逻辑综合  
- 对存储器或对原有值保存的需求，并且是边沿敏感行为  
- 6.6 确定状态机的综合  
- 显式声明的，类型为reg状态寄存器  
- 序列检测器  
- 6.7 寄存器逻辑  
- 寄存器类型变量：利用时钟信号同步进行赋值的变量  
- 6.8 状态编码  
- 时序机sequential machine  
- 寄存器，代表公共时钟D触发器组形成的一个存储结构  
- 复位  
- ***disable***  
- 与复位信号相关的行为中的第一个第一个语句必须是在复位信号有效时能够终止行为执行的一个条件语句。  
- 门控时钟  
- 
...
- 控制为主的系统和控制为主的系统  
- 数据通道单元：计算部分、数据通过系统的逻辑、数据在计算单元和内部寄存器之间移动的逻辑、移动数据进出外部系统的数据通道  
- 时序状态机  
- RISC状态机：处理器、控制器、存储器  
- ALU  
- 控制器  
...
- 基于ROM的状态机：ROM+状态寄存器  
- 2. 闪存flash  
- 有附加的内嵌电路、对晶体管阈值电压进行编码10^5次寿命  
- 3. SRAM，晶体管和电容构成，不需要刷新
- DRAM，刷新电路，
- 大型SRAM使用两层解码被重新组织成近似正方形 结构  
- 8.3 可编程逻辑阵列PLA  
- 两层阵列，与操作+或操作  
- 8.4 可编程阵列逻辑PAL  
- PLD  
- CPLD  
	- PLD阵列  
	- LAB逻辑阵列模块，（36输入、16输出、16宏单元（逻辑阵列、驱动或门的乘积项选择矩阵和可编程触发器））  
	- PIA可编程互联阵列  
	- IOB可编程模块阵列  
- 8.9 FPGA  
	- 基于沟道连接  
...
- LAB包含LE，逻辑单元  
- LE包括4输入LUT，一个可编程寄存器和专用进位级联链路  
- TTL晶体管-晶体管逻辑  
- EAB数组模块阵列，包含2kRAM存储器  
- ESB内置存储模块  
- FIFO  
- xilinx  
	- 组成：构造逻辑模块阵列CLB，（函数发生器F\G\H）
	- 全局布线资源，开关矩阵  
	- IOB  
	- SRAM存储 
	- JTAG，IEEE 1149.1，  
