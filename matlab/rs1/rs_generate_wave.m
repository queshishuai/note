...
function [Status] = rs_generate_wave( InstrObj, IQinfo_user, ...
    ...
    if (~IQinfo.no_scaling)
        ...
        max_IQ_data = max(abs(IQinfo.I_data + j*IQinfo.Q_data));
        peak = max_IQ_data;
        ...
        if (max(abs(IQinfo.I_data + j*IQinfo.Q_data))>1.0)
    ...
    len     = num2str(length(IQ_data)*2 + 3);

    % open file for write (binary mode)
%     WV_file_id = fopen(IQinfo.filename,'wb');
      WV_file_id = fopen(strcat(IQinfo.path,'\TM3_1A.wv'),'wb');
    if (WV_file_id < 0)
